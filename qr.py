...
# Tema renkleri
BG_COLOR = "#E8F0F2"      # Arka plan (pastel krem)
BTN_COLOR = "#3D405B"     # Buton rengi (koyu mavi/gri)
BTN_TEXT = "#F4F1DE"      # Buton yazısı (açık renk)
FONT = ("Segoe UI", 11)
...
def generate_qr_gui():
    ...
    messagebox.showinfo("Başarılı", "QR kod oluşturuldu ve gösterildi!")
...
Label(root, text="Metin veya URL Gir:", font=FONT, bg=BG_COLOR).pack(pady=(20, 5))

entry = Entry(root, width=35, font=FONT, relief="solid", bd=1)
entry.pack(pady=5)

styled_button(root, "QR Kod Oluştur", generate_qr_gui).pack(pady=15)

qr_label = Label(root, bg=BG_COLOR)
qr_label.pack(pady=10)

styled_button(root, "Kaydet", save_qr).pack(pady=10)

# Çıkış butonu opsiyonel
# styled_button(root, "Çıkış", root.quit).pack(pady=10)
